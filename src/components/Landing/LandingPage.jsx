  ...
  return (
    <div className="min-h-screen bg-[#070b18] text-slate-100 font-sans overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          ...
          <div className="rounded-3xl border border-cyan-500/20 bg-[#0a1024]/80 p-8 sm:p-10 shadow-[0_0_40px_rgba(34,211,238,0.1)]">
            ...
            <button
                ...
                className="w-full mt-6 py-3.5 rounded-xl bg-violet-600 text-white font-semibold text-sm transition-all duration-300 hover:bg-violet-500 hover:shadow-[0_0_25px_rgba(139,92,246,0.4)]"
              >
                {t('landing.studentSummary')}
              </button>
            </div>
        </div>
      </section>

      {/* ===== MODAL RESUMEN ESTUDIANTE ===== */}
      {modalOpen && studentData && (
        ...
        >
          <div
            className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-cyan-500/30 bg-[#0a1024] p-8 shadow-[0_0_60px_rgba(34,211,238,0.3)]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-2xl font-extrabold text-white">{t('landing.studentSummary')}</h3>
                <p className="text-xs text-slate-400 mt-1">{studentData.student?.name || studentData.full_name || '-'}</p>
              </div>
              <button
                onClick={() => setModalOpen(false)}
                className="w-10 h-10 rounded-xl border border-cyan-500/30 text-slate-300 hover:text-white hover:bg-slate-800 flex items-center justify-center transition"
                aria-label="Cerrar"
              >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <InfoCard label={t('landing.lessonsCompleted')} value={studentData.total_lessons_completed ?? studentData.lessons_completed ?? 0} />
              <div className="rounded-xl p-3 text-center border border-cyan-500/15 bg-slate-800/50">
                ...
                <div className="text-xl font-bold" style={{ color: getScoreColor(studentData.average_score ?? studentData.average ?? 0) }}>
                  {(studentData.average_score ?? studentData.average ?? 0).toFixed(1)}
                </div>
              </div>
            </div>

            <div className="mt-3">
              <InfoCard label={t('landing.currentStreak')} value={`${studentData.current_streak ?? studentData.streak ?? 0}d`} />
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-cyan-500/15 flex flex-col sm:flex-row gap-3">
              <button
                onClick={() => { setModalOpen(false); setStudentData(null); setDni(''); setCaptchaAnswer(''); fetchCaptcha(); }}
                className="flex-1 px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-400 to-blue-600 text-white font-bold transition-all duration-300 hover:shadow-[0_0_25px_rgba(34,211,238,0.4)]"
              >
                ← {t('landing.modalBack')}
              </button>
              <button
                onClick={() => setModalOpen(false)}
                className="flex-1 px-6 py-3 rounded-xl border border-cyan-500/30 text-slate-300 hover:bg-slate-800 transition"
              >
                {t('landing.ctaDemo')}
              </button>
            </div>
